...
class NearestEmbedFunc(Function):
    ...
    def backward(ctx, grad_output, argmin=None):
        ...
        if ctx.needs_input_grad[1]:
            ...
            idx_choices = (argmin.view(-1, 1) == latent_indices.view(1, -1)).type_as(
                grad_output.data
            )
            ...
            grad_emb = torch.sum(
                ...
                * idx_avg_choices.view(-1, 1, ctx.num_emb),
                0,
            )
...
class NearestEmbedEMA(nn.Module):
    ...
    def forward(self, x):
        ...
        result = (
            ...
            .index_select(0, argmin.view(-1))
            .view(shifted_shape)
            .permute(0, dims[-1], *dims[1:-1])
        )

        if self.training:
            latent_indices = torch.arange(self.n_emb).type_as(argmin)
            emb_onehot = (argmin.view(-1, 1) == latent_indices.view(1, -1)).type_as(
                x.data
            )
