...
class GPT(nn.Module):
    ...
    def __init__(self, config, model_type="actor"):
        ...
        self.state_encoder = nn.Sequential(
            nn.Linear(self.state_size, config.n_embd), nn.Tanh()
        )
...
class Trm_Dec(nn.Module):
    def __init__(self, hidden_size, obs_size):
        super(Trm_Dec, self).__init__()
        self.fc1 = nn.Linear(400, obs_size)
        # self.fc1 = nn.Linear(200, obs_size)
        # self.fc2 = nn.Linear(hidden_size, 1)
        self.relu = nn.ReLU()

    def forward(self, hidden):
        # print("hidden shape", hidden.size())
        prime_o = self.relu(self.fc1(hidden))
        # prime_a = self.relu(self.fc2(hidden))
        return prime_o
