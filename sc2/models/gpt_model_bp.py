...
class GPT(nn.Module):
    ...
    def forward(self, states, pre_actions, rtgs=None, timesteps=None):
        ...
        )  # (batch * block_size/btach * other_number, n_embd)
        ...
        if pre_actions is not None and self.model_type == "reward_conditioned":
            # todo(Muning) check here =============== begin
            rtg_embeddings = self.ret_emb(
                rtgs.type(torch.float32)
            )  # cancle the unsqueeze(-1)
            action_embeddings = self.action_embeddings(
                pre_actions.type(torch.long).squeeze(-1)
            )  # (batch, block_size, n_embd)
            token_embeddings = torch.zeros(
                (states.shape[0], states.shape[1] * 3, self.config.n_embd),
                dtype=torch.float32,
            ...
            pre_actions is None and self.model_type == "reward_conditioned"
            ...
            pre_actions is None and self.model_type == "naive"
        ):  # only happens at very first timestep of evaluation
        ...
        position_embeddings = (
            torch.gather(
                ...
                torch.repeat_interleave(
                    timesteps.unsqueeze(-1), self.config.n_embd, dim=-1
                ),
            )
            + self.pos_emb[:, : token_embeddings.shape[1], :]
        )  # position embedding for the state and reward is same for the repeat operation
        # todo(Muning) check here =============== end
