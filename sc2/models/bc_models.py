...
class BCConfig:
    ...
    def __init__(
        ...
        discrete_actions_sections: Tuple = (0,),
        num_hiddens: Tuple = (64, 64),
        activation: str = "relu",
        **kwargs,
    ):
...
class BC(nn.Module):
    def __init__(self, config):
        ...
        assert sum(config.discrete_actions_sections) == self.num_discrete_actions, (
            f"Expected sum of discrete actions's "
        ...
        self._layers_units = [
            (_module_units[i], _module_units[i + 1])
            for i in range(len(_module_units) - 1)
        ]
        ...
        for idx, module_unit in enumerate(self._layers_units):
            ...
            self._module_list.add_module(
                f"Layer_{idx + 1}_Linear", nn.Linear(n_units_in, n_units_out)
            )
            if idx != len(self._layers_units) - 1:
                self._module_list.add_module(
                    f"Layer_{idx + 1}_Activation", activation()
                )
        if self.num_discrete_actions:
            self._module_list.add_module(
                f"Layer_{idx + 1}_Custom_Softmax",
                MultiSoftMax(
                    0, self.num_discrete_actions, self.discrete_action_sections
                ),
            )
    ...
    def forward(self, states, actions, targets=None, rtgs=None, timesteps=None):
        ...
        if targets is not None:
            loss = F.cross_entropy(
                ...
                torch.tensor(targets, dtype=torch.long).reshape(-1),
            )  # maybe we can multiply the rtgs on the loss
        return logits, loss
...
class MultiSoftMax(nn.Module):
    ...
    def __init__(self, dim_begin: int, dim_end: int, sections: Tuple = None):
        ...
        if sections:
            assert dim_end - dim_begin == sum(sections), (
                "expected same length of sections and customized" "dims"
            )
